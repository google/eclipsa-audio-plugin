...
TEST(LoggerTest, FileRetentionPolicy) {
  ...
  // Clean up existing log files before the test
  std::vector<std::string> existingLogFiles = Logger::getInstance().getLogFilePaths();
  for (const auto& file : existingLogFiles) {
    std::remove(file.c_str());
  }
  ...
  ASSERT_LE(logFiles.size(), 6u) << "Expected at most 6 log files (5 + 1 temporary during rotation).";
  ASSERT_GE(logFiles.size(), 5u) << "Expected at least 5 log files after retention policy.";
  ...
  for (const auto& filePath : logFiles) {
    ...
    std::string numStr = fname.substr(underscorePos + 1, dotPos - underscorePos - 1);
    try {
  ...
  EXPECT_GT(fileNumbers[0], 5) << "Expected oldest files to be deleted; min file number should be > 5.";
  
  // Verify files are consecutive (no gaps)
  for (size_t j = 1; j < fileNumbers.size(); ++j) {
    EXPECT_EQ(fileNumbers[j], fileNumbers[j - 1] + 1) << "File numbers are not consecutive.";
  }
...
TEST(LoggerTest, FileRetentionDuringActiveLogging) {
  ...
  EXPECT_GT(fileNumbers[0], 0) << "Oldest file should have been deleted during rotation.";
