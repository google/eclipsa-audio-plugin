...
void GainProcessor::processBlock(juce::AudioBuffer<float>& buffer,
  ...
  for (int i = 0; i < juce::jmin(static_cast<int>(channelGainsDSP_.size()), static_cast<int>(gains_.size())); i++) {
    setGain(i, gains_[i]->get());
  }

  for (int i = 0; i < juce::jmin(buffer.getNumChannels(), static_cast<int>(channelGainsDSP_.size())); i++) {
